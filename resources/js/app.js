document.addEventListener('DOMContentLoaded', function() {
    ...
    if (volumeInputs.length > 0 && volumeSelectorBg) {
        // Установка начального положения фона
        const checkedInput = document.querySelector('input[name="volume"]:checked');
        if (checkedInput) {
            const index = Array.from(volumeInputs).indexOf(checkedInput);
            const width = 100 / volumeInputs.length;
            volumeSelectorBg.style.transform = `translateX(${index * width}%)`;
            volumeSelectorBg.style.width = `${width}%`;
        }
        ...
        volumeInputs.forEach((input, index) => {
            input.addEventListener('change', function() {
                ...
                // Update the position of the white background
                const width = 100 / volumeInputs.length;
                volumeSelectorBg.style.transform = `translateX(${index * width}%)`;
                volumeSelectorBg.style.width = `${width}%`;
            });
